...
class OPTIONALHEADER:
	...
	def __init__(self,data):
		...
		if self.is64Bit:
			self.ImageBase=unpackuint64(data[24:32]) #64bit doesn't have BaseOfData
		else:
		...
		if self.is64Bit:
			self.SizeOfStackReserve=unpackuint64(data[72:80]) #in case of 64bit we have a shift in offset
		...
		if certificaterva>0:
			self.DataDirectory["CERTIFICATE"]=[certificaterva,certificatesz,None]
		...
		if architecturerva>0:
			self.DataDirectory["ARCHITECTURE"]=[architecturerva,architecturesz,None]
		...
		if boundimportrva>0:
			self.DataDirectory["BOUNDIMPORT"]=[boundimportrva,boundimportsz,None]
		...
		if delayimportrva>0:
			self.DataDirectory["DELAYIMPORT"]=[delayimportrva,delayimportsz,None]
...
class SECTIONHEADER:
	_characteristictypes={
					...
					0x01000000:	"IMAGE_SCN_LNK_NRELOC_OVFL", #if this is set the count is stored in VA of the first Entry in Relocs not NumberOfRelocations
...
class EXPORTTABLE:
	def __init__(self,data,tables_fileoffset,sections):
		self.exportTable = []
		self.ExportFlags = unpackuint32(data[tables_fileoffset+0:tables_fileoffset+4])
		self.TimeStamp = unpackuint32(data[tables_fileoffset+4:tables_fileoffset+8])
		self.MajorVersion=unpackuint16(data[tables_fileoffset+8:tables_fileoffset+10])
		self.MinorVersion=unpackuint16(data[tables_fileoffset+10:tables_fileoffset+12])
		self.NameRVA= unpackuint32(data[tables_fileoffset+12:tables_fileoffset+16])
		self.OrdinalBase= unpackuint32(data[tables_fileoffset+16:tables_fileoffset+20])
		self.NumberOfFunctions= unpackuint32(data[tables_fileoffset+20:tables_fileoffset+24])
		self.NumberOfNames= unpackuint32(data[tables_fileoffset+24:tables_fileoffset+28])
		self.FunctionsRVA= unpackuint32(data[tables_fileoffset+28:tables_fileoffset+32])
		self.NamesRVA= unpackuint32(data[tables_fileoffset+32:tables_fileoffset+36])
		self.OrdinalsRVA= unpackuint32(data[tables_fileoffset+36:tables_fileoffset+40])
		...
		for i in range(self.NumberOfNames):
			NamesAndOrdinals[unpackuint16(data[OrdinalsFileOffset+i*2:OrdinalsFileOffset+i*2+2])]=unpackuint32(data[NamesFileOffset+i*4:NamesFileOffset+i*4+4])

		#create export table
		for i in range(self.NumberOfFunctions):
			FunctionAddress = unpackuint32(data[FunctionsFileOffset+i*4:FunctionsFileOffset+i*4+4])
			...
			if i in NamesAndOrdinals.keys():
				...
				self.exportTable.append(ExportObject(i+self.OrdinalBase,FunctionAddress,i,NamesAndOrdinals[i],symbol))# Ordinal,address,hintg,string address, string
				...
				if namesoffset > tables_fileoffset:
					symbol=readstring(data, namesoffset)
				self.exportTable.append(ExportObject(i+self.OrdinalBase,FunctionAddress,0,0,symbol)) # Zeroes because either exported by Ordinal or has forwarder RVA
...
class IMPORTTABLE:
	def __init__(self, data,tables_fileoffset,sections,mode64):
		...
		while True:
			...
			iltoffset = convertRVAToOffset(importdirectorytable.ILTRVA, sections)
			...
			while True:
				...
				value=0
				if mode64:
					iltentry = unpackuint64(data[iltoffset+j*8:iltoffset+j*8+8]) # 8 bytes
					importbyordinal=iltentry&0x8000000000000000 # is the import by ordinal bit set?
					...
					iltentry = unpackuint32(data[iltoffset+j*4:iltoffset+j*4+4]) # 4 bytes
				...
				if importdirectorytable.ForwarderChain!=0 and importdirectorytable.ForwarderChain!=-1:
					...
					importObjects.append(ImportObject(iatrva,0,0, forwarerstring,importdirectorytable.ForwarderChain,value)) #since we have a  forwarder set it to forwarderchain

				#import by ordinal
				elif importbyordinal:
					importObjects.append(ImportObject(iatrva,iltentry&0xFFFF,0,0,"",0,value)) #no hint no name

				#import by name
				else:
					nametableoffset=convertRVAToOffset((iltentry&0x7FFFFFFF),sections)

					hint = unpackuint16(data[nametableoffset:nametableoffset+2]) #read hint
	...
	def __init__(self, data, tables_fileoffset, sections, mode64):
		...
		while True:
			...
			while True:
				...
				value=0
				if mode64:
					intentry = unpackuint64(data[namesoffset+8*j:namesoffset+8*j+8])
					importbyordinal=intentry&0x8000000000000000 # is the import by ordinal bit set?
					iatrva= delayIAT+j*8
					valueoffset=convertRVAToOffset(iatrva,sections)
					value=unpackuint64(data[valueoffset:valueoffset+8])
				else:
					intentry = unpackuint32(data[namesoffset+4*j:namesoffset+4*j+4])
				...
				#import by ordinal
				if importbyordinal:
					importObjectsappend(ImportObject(iatrva,intentry&0xFFFF,0,0,"",0,value)) #no hint no name

				#import by name
				else:
					nametableoffset=convertRVAToOffset((intentry&0x7FFFFFFF),sections)
					hint = unpackuint16(data[nametableoffset:nametableoffset+2]) #read hint
					name = readstring(data, nametableoffset+2)
					importObjectsappend(ImportObject(iatrva,0,(intentry&0x7FFFFFFF),hint, name, 0,value))
				j=j+1
	...
	def __init__(self, data, tables_fileoffset, sections):
		...
		while True:
			...
			for j in range(countofcodes):
				codes.append(unpackuint16(data[codesoffset+j*2:codesoffset+2+j*2]))


			handler= None

			if flags &0x3: #1 - 3 we have a handler
				handlerreloffset = (countofcodes%2+countofcodes)*2#the rva of the handler must be aligned after the code
				handlerinfooffset= codesoffset+handlerreloffset
				exceptharva = unpackuint32(data[handlerinfooffset:handlerinfooffset+4])

				count = unpackuint32(data[handlerinfooffset+4:handlerinfooffset+8])
				...
				if count <5:
					...
					for i in range(count):
						...
						startrva=unpackuint32(data[scoptableoffset+scopes:scoptableoffset+scopes+4])
						endrva= unpackuint32(data[scoptableoffset+4+scopes:scoptableoffset+8+scopes])
						handlerrva=unpackuint32(data[scoptableoffset+8+scopes:scoptableoffset+12+scopes])
						jumptarget= unpackuint32(data[scoptableoffset+12+scopes:scoptableoffset+16+scopes])
			...
			object.UnwindInfo = UnwindInfoObject(version, flags, szprolog, countofcodes, frameregister, frameregisteroff, codes, handler)
...
class PeParser:
	...
	def __init__(self, buffer):
		try:
			...
			self.NTHEADER_ =  NTHEADER(self.buffer[self.DOSHEADER_.lfanew:]) #next header at offset field
			...
			offset = self.DOSHEADER_.lfanew+20+4+224				  # FL offset + FL_SIZE+ PESignature_SIZE+PEHeader_SIZE
			...
			for i in range(numberofentries):
				self.SECTIONHEADERS_.append(SECTIONHEADER(self.buffer[offset+40*i:]))
			...
			for i in  self.NTHEADER_.OPTIONALHEADER_.DataDirectory:
				...
				for j in range(numberofentries):
					...
					if dd_rva >=section_rva and dd_rva <= section_rva+section_size:
						self.NTHEADER_.OPTIONALHEADER_.DataDirectory[i][2]=self.SECTIONHEADERS_[j]
						break

			#Decode directories
			for key in self.NTHEADER_.OPTIONALHEADER_.DataDirectory:
				tablesrva = self.NTHEADER_.OPTIONALHEADER_.DataDirectory[key][0]			  #get rva of the data directory
				section = self.NTHEADER_.OPTIONALHEADER_.DataDirectory[key][2]  			  #get corresponding section in which the information resides
				tables_fileoffset = tablesrva-section.VirtualAddress+section.PointerToRawData #offset to directory table: internal use

				# parse directories
				if key == "EXPORT":
					log("pypeparser.py#Parsing EXPORT")
					self.EXPORTTABLE_=EXPORTTABLE(self.buffer,tables_fileoffset,self.SECTIONHEADERS_)

				elif key =="IMPORT":
					log("pypeparser.py#Parsing IMPORT")
					self.IMPORTTABLE_=	IMPORTTABLE(self.buffer,tables_fileoffset, self.SECTIONHEADERS_, self.NTHEADER_.OPTIONALHEADER_.is64Bit)

				elif key =="DELAYIMPORT":
					log("pypeparser.py#Parsing DELAYIMPORT")
					self.DELAYIMPORTTABLE_=DELAYIMPORTTABLE(self.buffer,tables_fileoffset,self.SECTIONHEADERS_, self.NTHEADER_.OPTIONALHEADER_.is64Bit)

				elif key =="TLS":
					log("pypeparser.py#Parsing TLS")
					self.TLS_=TLS(self.buffer, tables_fileoffset, self.NTHEADER_.OPTIONALHEADER_.is64Bit)

				elif key =="EXCEPTION":  #x64 only, on x86 the exception information is saved on the stack
					log("pypeparser.py#Parsing EXCEPTION")
					self.EXCEPTIONTABLE_ = EXCEPTIONTABLE(self.buffer,  tables_fileoffset, self.SECTIONHEADERS_ )
	...
	def write_all(self, folder):
		...
		for i in range(len(codesections)):
			...
			with open(folder+"\\"+section_files_to_disassemble[i]+".bin", "wb") as fd: # need to investigate which limits are good
	...
	def GetSections(self):
		NumberOfEntries=self.NTHEADER_.FILEHEADER_.NumberOfSections
		out="NumberOfSections:"+str(NumberOfEntries)
		out2=""
		for i in range(NumberOfEntries):
			out2="\n\tName\t\t\t:\t%s\n\tVirtualAddress\t:\t0x%x\n\tVirtualSize\t\t:\t0x%x\n\tPointerToRawData:\t0x%x\n\tSizeOfRawData\t:\t0x%x\n\t%s" %(self.SECTIONHEADERS_[i].Name,self.SECTIONHEADERS_[i].VirtualAddress,self.SECTIONHEADERS_[i].VirtualSize,self.SECTIONHEADERS_[i].PointerToRawData,self.SECTIONHEADERS_[i].SizeOfRawData,self.SECTIONHEADERS_[i].GetCharacteristics())
			out=out+out2
		out=out+"\n"
		return out
		...
		for i in range(NumberOfDLLs):
				...
				for j in range(numberofobjects):
					...
					if table.importObjects[j].Ordinal:
						name ="__importedByOrdinal_%x"%(table.importObjects[j].Ordinal)
					else:
						name ="%s"%(table.importObjects[j].Name)

					delayimport_dic[table.importObjects[j].IATAddressRVA+base]=table.nameOfDLL+"!"+name
					delayimport_stubs_dic[table.importObjects[j].value] = table.nameOfDLL+"!"+name+"__stub"
	...
	def GetCodeSections(self):
		...
		for i in range(NumberOfSections):
			if (self.SECTIONHEADERS_[i].Characteristics & 0x20000020) >0: #either code or executable
		...
		for i in range(NumberOfDLLs):
			...
			for j in range(numberofobjects):
				...
				if table.importObjects[j].Forwarder:
					...
					name ="%s"%(table.importObjects[j].Name)
				import_dic[table.importObjects[j].IATAddressRVA+base]=table.nameOfDLL+"!"+name
				out=out+address+"\t"+name+"\n"
	...
	def GetExceptions(self):
		...
		for i in range(NumberOfExceptions):
			...
			out2="\tVersion:%x \tFlags:%s \tCountOfCodes:%d \tFrameRegister:%s \tFrameOffset:0x%x"%(unwindinfo.Version, unwindinfo.getFlags(),unwindinfo.CountofCodes, unwindinfo.getFrameRegister(), unwindinfo.FrameOffset)
			...
			if flags == 0x4:	#chained
				...
				rva = "Handler Address:0x%x"%(entry.UnwindInfo.ExceptionHandler.HandlerRVA+base)
				...
				if nmofscopetables <5: #dunno how to recognize if it's data or scopetables
					...
					for i in range(nmofscopetables):
						...
						out4=out4+"\t\tScope Table:\n\t\t\tStart:0x%x\n\t\t\tEnd:0x%x\n\t\t\tHandler Address:0x%x\n\t\t\tJump Target:0x%x"%(table.StartRVA+base,table.EndRVA+base,table.HandlerRVA+base, table.JumpTarget+base)
				else:
