...
int main(int argc, char *argv[])
{
   ...
   else
   {
      ...
      LogNormal("main: Test[9]: Set 0x100 - 0x120 to 0xFF");
      i32MemoryAdd(&xMemory, 0x100, 16, &rui8TestArrayFfs[0]);
      i32MemoryAdd(&xMemory, 0x111, 16, &rui8TestArrayFfs[0]);
      vMemoryPrint(&xMemory);

      LogNormal("main: Test[10]: Remove region 0x100 -  0x150");
      i32MemoryCopy(&xMemory, 0x00, 0x108, 0x20);
      vMemoryPrint(&xMemory);
